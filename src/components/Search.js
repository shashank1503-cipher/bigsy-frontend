...
const Search = () => {
  ...
  const fetchData = async (dataType = 0) => {
    ...
    const url = `http://127.0.0.1:8000/search?q=${toSearch}&page=${page}&filters=${JSON.stringify(
      filters
    )}`;
  ...
  return (
    <>
      ...
      >
        <Flex gap={2} margin={"auto"} minW={"55%"}>
          <Input
            placeholder="Search"
          ...
          >
            ...
            >
              {totalPages?.map((p) => {
                ...
                else if (p < page - 2) return <></>;
                else if (p > page + 3) return <></>;
